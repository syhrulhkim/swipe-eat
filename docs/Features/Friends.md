...
somebody. The shipped copy says what is actually true instead (D121) — the
...
4. `match_contacts` peppers the hex with a vault secret and hashes again before
   ...
   lives for the length of one statement (D122).
...
(D122): `handle_new_user` and `sync_phone_hash` are both `after insert`
...
One row per pair, in one direction (D124):
...
- **update/delete on a blocked row** — the blocker only (D124). See §4.

### RPCs

| Function | Returns | Why definer |
|---|---|---|
| `get_friends()` | accepted friends, by name | `profiles` is owner-only (D123) |
...
| `get_plan_votes(bigint)` | the tally, with faces | definer (D123) |

`friend_request` is one RPC with an action rather than four functions (D125).
...
| ID | Decision | Status |
|---|---|---|
| D121 | Onboarding says what contact matching actually does. The design's *"matching happens on your phone"* cannot be true of any scheme that finds strangers, so the shipped copy promises what the code delivers: the numbers leave scrambled, the names never leave, nothing is kept. | locked 2026-09-06 |
| D122 | The matching path never sees a phone number, and the hash lives in `phone_hashes`, not on `profiles`. Client normalises and SHA-256s; the server peppers from the vault and hashes again; `match_contacts` writes and keeps nothing. The side table avoids a trigger-ordering race on `auth.users` and keeps a secret-ish column off the table every screen selects from. | locked 2026-09-06 |
| D123 | Every cross-user read returns exactly three columns — id, name, avatar url — through a `security definer` function. `profiles` stays owner-only; no policy is ever opened to make a name visible. | locked 2026-09-06 |
| D124 | A friendship pair is one row in one direction, keyed `(user_lo, user_hi)` with `user_lo < user_hi`; `requester_id` records who asked and the *other* party accepts. A blocked row is the blocker's alone to update or delete, and a send into a block returns the same neutral error a genuine failure gives. | locked 2026-09-06 |
| D125 | One `friend_request(user_id, action)` RPC rather than four functions. The five verbs are the same statement against the same primary key, and four functions would be four places to get the pair ordering wrong. Security invoker, so RLS stays the boundary. | locked 2026-09-06 |

D126 (a plan member may see the other members), D127 (time voting needs no new
schema) and D128 (voting gets a screen the design does not draw) are the plan's
