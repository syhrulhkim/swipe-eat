...
-- columns everything cross-user is allowed to see (D121-D125).
--
...
--     is the fact acceptance needs: the *other* party accepts (D124).
--   * **Cross-user reads return three columns and no more** (D123): id, name,
...
--   * **The matching path never sees a phone number** (D122). The client
...
--     no scheme that finds strangers can honestly promise (D121).
--   * **The hash lives in a side table, not on `profiles`** (D122). Its
...
create table if not exists public.friendships (
  -- Ordered pair (D124): the smaller uuid is always `user_lo`, so one pair is
...
-- ---------------------------------------------------------------------------
--
-- One RPC with an action rather than four (D125). Send, accept, decline and
