...
--
--   * **Voting needs no new schema** (D133). A member upserts their own row in
...
--   * **A plan member may now see the other members** (D132). Phase 7 gave a
...
--   * **Names still come through definer functions** (D129). The new policy
...
-- (D129): `profiles` is owner-only, so names and faces cannot be joined by an
...
-- (D129); gated on membership or ownership inside, so a plan you have nothing
