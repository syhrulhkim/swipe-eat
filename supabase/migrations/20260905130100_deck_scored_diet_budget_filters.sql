-- deck_scored gains the three hard rules the profile's diet & budget answers
-- imply (D105). Applied to the live project as the
-- `deck_scored_diet_budget_filters` migration; kept here verbatim so a local
-- stack ends up with the same function body.
--
-- Same signature as before, so this really is a replace — no drop needed, and
-- get_deck, which calls it, is untouched.
--
...
--                cannot. Most of the catalogue is unknown, which is exactly
...
--   budget_max   an unknown `price_from` passes. 1 419 of 1 605 live rows have
--                no price, so dropping them would empty the deck for anyone
--                who answered the budget question at all.
